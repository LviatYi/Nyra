...
mod build_tesseract {
        ...
        F: FnOnce(),
    {
        ...
        let mut linked_lib_name: Option<String> = None;
        ...
        if cached_path.exists() {
            ...
            if let Err(e) = fs::copy(&cached_path, &out_path) {
                ...
                build_fn();
            } else if let Some(lib_name) = cached_path.file_stem().and_then(|s| s.to_str()) {
                linked_lib_name = Some(lib_name.to_string());
            }
        } else {
            ...
            if let Some(lib_path) = found_lib_path {
                if let Some(lib_name) = lib_path.file_stem().and_then(|s| s.to_str()) {
                    linked_lib_name = Some(lib_name.to_string());
                }
        ...
        if let Some(lib_name) = linked_lib_name {
            println!("cargo:warning=Linking {} as {}", name, lib_name);
            println!("cargo:rustc-link-lib=static={}", lib_name);
        } else {
            println!("cargo:warning=Falling back to default link name for {}", name);
