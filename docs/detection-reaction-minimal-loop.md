...
日期：2026-09-10；2026-09-11 补充 TypeScript 接入成本估算。性质：设计分析与需求拆分，尚未实现或完成目标应用实测。
...
结合本会话后续补充的游戏 UI 异步刷新和宏开发体验需求，建议调整为 **Windows 窗口捕获 + 可供执行流程等待的 Detection 条件 +
全局确认/取消快捷键 + 成熟语言 SDK + Nyra 执行服务**。 宏需要支持“执行动作 → 等待可核对的后置条件 →
继续”，不能只依赖预设等待时间。语言优先考察 TypeScript，具体选择尚未定案，见第 10 节。

第 2～9 节保留首次分析的基础方案，其中 JSON 动作列表、单检测器配置及对应拆分属于初版设计；涉及宏表达、运行中检测和调试的建议以第
10 节修订为准。JSON 仍适合配置数据，不再作为复杂宏的首选编写形式。
...
本节记录有限动作序列这一初始假设下的选型。后续需求要求运行中等待画面条件，并重视编码、调试体验，因此以下“首版首选”判断已由第
10 节调整，不是当前最终推荐。
...
用户反馈了两项实际问题：游戏在输入后需要不固定的 UI 刷新时间，后续动作可能提前发生；AHK
的编码和调试体验也未满足需求。这意味着此前“有限按键序列 + 固定等待”的假设不够充分。

AHK 本身并非不能观察画面：其 `ImageSearch`
可以查找屏幕区域中的图像，脚本可以据此轮询等待。因此问题主要在于目前流程缺少可靠的状态反馈及配套开发工具，换掉语法不会自动解决。[AutoHotkey 官方文档源码：ImageSearch](https://raw.githubusercontent.com/AutoHotkey/AutoHotkeyDocs/v2/docs/lib/ImageSearch.htm)
...
宏可以继续写成普通顺序代码，底层执行器维护 Running / Waiting / Completed / Cancelled / Failed
等状态，不要求用户手写状态转移表。固定等待仍可用于按住时长、最小输入间隔等确定要求；对于页面准备情况，优先等待具体状态。

### 10.2 Detection 应提供什么

Detection 从“通知初始条件达成”扩展为宏可以查询和等待的观测服务。Reaction 负责步骤顺序、超时与失败决策；Detection
负责证据和匹配语义，两者通过有明确生命周期的等待请求协作。
...
动作之后的新帧也可能仍显示旧 UI。若条件在动作前已经成立，仅要求 `afterAction` 不够；应使用能区分新旧状态的条件，例如“已选中项目
B”，或在确有必要时要求观察到“旧状态离开 → 新状态到达”。不能对所有步骤一律要求先 false 再 true，因为目标本来已经就绪时，这会制造无谓超时。

另一个细节是订阅时机：应由 Nyra
在动作发送前建立等待观测边界，动作完成后开放满足判定，使用有界观测缓冲避免“动作已完成，但等待还没注册”的间隙。传回脚本的动作回执是输入发送回执，不是游戏确认回执。捕获时间、动作时间应由宿主统一到可比较的时钟；若捕获后端无法证明帧顺序和新鲜度，不能宣称已经排除旧帧。
...
JSON 适合目标选择、模板、阈值、热键等数据。流程出现等待、分支、局部变量和错误处理后，JSON 会逐步变成难以阅读的程序；自建文本 DSL
则需要维护补全、类型检查、断点和错误定位。建议暂不投入这两条路线。
...
在当前明确强调编码、调试体验的前提下，优先考察 TypeScript SDK。SDK 的声明文件可描述动作参数和返回类型，Node.js 有现成
Inspector 调试接口；Nyra
不必自行实现一门语言的调试器。但还需要提供运行入口、源码映射（采用转译时）与编辑器连接方式，不能把“语言支持调试”当作“接入零成本”。[TypeScript：声明文件](https://www.typescriptlang.org/docs/handbook/declaration-files/introduction.html)、[Node.js：调试](https://nodejs.org/learn/getting-started/debugging)

“无需编译”在这里指用户编辑脚本后直接运行，不需要重建 Nyra 或手工执行构建步骤；如果选择的 TS
语法需要转译，由配套运行器负责。若不接受额外运行进程，则优先重新评估 Lua，而不是直接实现 JS 引擎嵌入。Lua
提供协程暂停/恢复和调试接口，但具体编辑器调试适配仍需单独验证。[Lua 5.4 手册：协程](https://www.lua.org/manual/5.4/manual.html#2.6)
...
`pressAndWait` 在宿主侧执行：预先建立观测请求、发送按键、等待动作后的指定条件，超时即失败。`require`
在真正发送前检查当前有效条件，而非依赖之前返回脚本的布尔值。示例中的稳定时长和超时都是待验证参数，最后一步仍只代表发送完成；如果能识别业务成功，再显式增加后置条件。

### 10.5 脚本运行与实际输入的职责分配

建议让脚本进程只编排流程，通过本地管道等通道调用 Nyra。真正的输入发送、持键记录、Detection 等待和全局取消都由 Rust
宿主执行。这与“启动 AHK 后由脚本自行发送所有按键”不同：即使脚本卡住，Nyra 仍能取消运行、拒绝后续请求并清理自身持有的键。

每次运行分配 `runId`，每个动作分配 `requestId`
。宿主只接受当前有效运行的请求，禁止并发输入和无界动作队列；取消先使运行失效，再停止动作并清理，迟到的请求和响应不能恢复已取消流程。通信中断视为运行失败；不自动重放可能已经发送的动作。

无响应脚本需要有界运行时间；开发时在输入安全边界暂停可以显式延长，但目标新鲜度、焦点检查和取消不能暂停。脚本运行器退出时也不能留下长期持键动作。使用通用语言执行的是受信任本地代码，SDK
约定本身不构成限制任意脚本访问系统的沙箱。

这套设计确实增加了运行器和本地协议的工作量，无法消除开发成本；收益在于将工作集中到 Nyra
特有的观测和执行语义，复用已有语言工具。首版只选一种语言，不建设多语言插件生态。
...
断点有一个外部自动化特有的限制：脚本暂停时游戏仍在运行。建议优先在没有持键、没有正在执行动作的步骤边界暂停；恢复前重新校验目标和所需条件，旧证据不能继续授权下一步。普通脚本断点不保证同时暂停
Rust 执行中的动作，Nyra 应清楚显示这种状态，取消热键始终有效。

离线回放也应准确限定：录下的
ROI/条件序列可用于复现匹配判断和等待逻辑，但不能证明另一组按键在真实游戏里会得到同样结果。仅“不发按键”的模式无法自然推进真实页面，需要人工推进页面或明确的记录输入，不能将其视为完整闭环验证。

值得借鉴的是 Playwright 的操作前条件检查、自动等待和 Trace Viewer 的动作/观测关联。这里只借鉴执行和诊断设计；Playwright
面向浏览器，其 DOM
和可操作性检查不能直接用来控制原生游戏。[Playwright：Auto-waiting](https://playwright.dev/docs/actionability)、[Playwright：Trace viewer](https://playwright.dev/docs/trace-viewer)
...
原第 8 节“独立宏执行”仍可先验证输入后端，但运行中的条件等待需要
Detection；不能将两个模块完全实现后才开始联调。配置也需要从“一个检测器与一段动作”调整为“场景条件集合 + 启动条件 + 脚本入口”。

补充验收重点：刷新快慢变化时等待正确；动作前旧匹配不能直接满足后置条件；Unknown
和断流不能累计稳定时长；超时不会继续下一步或重复发出动作；取消和脚本断开后不执行迟到请求；断点恢复会重新检查实际页面。以上仍是设计要求，本次未实现
SDK、修改业务代码或新增测试。

## 11. TypeScript 接入成本估算

### 11.1 估算结论和边界

如果采用现成 Node.js 运行时、已有 IDE 调试器和一个很薄的 Nyra SDK，TypeScript 接入本身是可控的中等工作量。以一名熟悉 Rust、TypeScript 和 Windows 进程操作的开发者计，建议按以下范围理解：

| 交付层级 | 累计投入估算 | 能达到的程度 |
|---|---|---|
| 技术验证原型 | 2～4 人日 | 加载 TS、一次宿主调用、等待响应、命中断点；尚不足以日常运行 |
| 开发者自用版本 | 10～18 人日 | 最小 SDK、可靠协议生命周期、异常和取消处理、基本类型检查、脚本调试和步骤诊断；使用本机已配置的运行时与编辑器 |
| 可分发的首版 | 13～23 人日 | 在自用版上增加随包运行时/SDK、版本检查、离线加载和干净 Windows 环境验证；不含安装器和自动更新 |

以上是设计阶段的工程判断，不是外部报价或已经测得的实现效率。1 人日约为 8 小时有效工作；原型包含在后续累计投入内，不重复相加。范围置信度中低，排期可在选定范围上另留约 20%～30% 余量，应对语言工具与 Windows 集成问题；不预设 AI 辅助能按固定比例缩短时间。

最关键的前提：**这些数字估算的是 TS 接入层，不是从当前仓库完成整个 Detection / Reaction 闭环的总工期。** 当前仓库尚无可用的捕获、检测和输入执行服务。底层目标兼容性、识别算法、动作后观测边界、按键清理、全局热键和浮窗状态改造仍需另计；选择 JSON、Lua 或 AHK 也不能省掉这些共同工作。

上述估算假定宿主已经有明确的动作、条件等待、取消及错误接口，TS 层负责接入和验证。可以先用受控的宿主返回验证协议，但那只能证明桥接成立。未拿到目标游戏、真实宏和观测样本前，不应把 TS 工期直接当作全项目承诺。

### 11.2 必要工作拆分

以下各行的边界按工作职责划分，总计 10～18 人日，不再另外叠加同范围的“联调费用”。

| 工作 | 估算 | 最小交付 |
|---|---|---|
| 接口协议与 Rust 桥接 | 2～3 人日 | 请求/响应格式、协议版本、参数校验、消息分帧、错误分类、runId/requestId、宿主接口映射 |
| TS SDK 与类型体验 | 1～2 人日 | `press`、`waitFor`、`pressAndWait`、`step` 的 Promise 封装，类型声明、错误对象及取消传播 |
| TS 加载与运行入口 | 1～2 人日 | 定位 Node、版本检查、加载指定宏和本地模块、导出入口校验、语法/类型错误提示、路径处理 |
| 现成 IDE 调试接入 | 1～2 人日 | Inspector 启动、手动 attach 配置、源码断点、异常堆栈、重启流程；先验收一种编辑器 |
| 运行生命周期可靠性 | 2～4 人日 | 崩溃/断线/超时、取消与迟到请求竞态、单运行约束、退出码与进程回收、调试等待与运行截止时间的区分 |
| 最小诊断与集成验证 | 3～5 人日 | 步骤时间线、等待原因、关联宿主证据、错误定位，以及正常/取消/断线/断点恢复路径的有界验证和必要使用说明 |

协议行负责定义和打通通道；生命周期行负责所有异常下的行为，二者不是两个独立 RPC 框架。诊断行只消费宿主已经能给出的观测证据；如果宿主没有截图或帧元数据能力，补齐该能力属于共同底层成本。

为控制第一版范围，条件名暂时运行前校验，不做从配置自动生成类型的工具；SDK 的静态方法和参数仍提供完整声明。截图先支持关联已有证据或按需保存单帧，不建设 Trace Viewer、录屏或离线执行回放。

### 11.3 协议可以很小，可靠执行语义不能省

推荐首版由 Rust 启动一个 Node 子进程，使用标准输入/输出上的逐行 JSON 交换消息，stderr 用于日志。这样无需建立 HTTP 服务、维护监听端口、引入 gRPC 或实现 Node 专用 IPC 协议。Node 文档也区分了普通标准流与其专用 IPC 通道，并说明标准流缓冲有限，必须持续消费输出；Rust 父进程同样需要妥善处理管道读写。[Node.js：Child process](https://nodejs.org/api/child_process.html)

最低消息概念可限定为：握手、请求、响应、步骤事件和运行结束。宏通过 SDK 发起方法调用，宿主返回结果或结构化错误；不需要通用远程对象、服务发现、重连续跑或跨版本兼容矩阵。只校验 SDK 与宿主的协议版本匹配，不匹配就拒绝启动。

即使使用逐行 JSON，仍须处理半条消息、合并读取、消息大小限制和慢消费者；标准输出保留给协议，运行器将普通 console 日志转到 stderr。日志污染协议时应明确失败，不能猜测修复。每次执行只允许一个待处理输入步骤，拒绝脚本通过并行调用积压多个动作。

真正容易低估的是如下语义：脚本取消时宿主动作可能已开始；宿主执行成功时返回通道可能已断；被取消的 Promise 不能保证宿主停止；迟到请求不能重新启动旧运行。因此必须由宿主记录运行有效性、驱动实际取消，不自动重放动作。这部分值得投入，单纯把函数名编码为 JSON 通常只占很小部分工作量。

每次运行创建新脚本进程，结束后回收，可简化模块缓存和脚本更新。由此产生的进程启动延迟应在原型中测量；若对确认后第一步的延迟不够用，再决定是否需要预启动，不在估算基线中默认维护进程池。

### 11.4 TS 直接运行：优先限定语法，复用 Node

无需自行开发 TypeScript 运行时。可以固定一个经过验证、支持原生类型擦除的 Node 版本，直接加载宏文件。Node 24.12.0 文档已经将该能力标为稳定；它支持可擦除的类型语法，但不执行类型检查，也不按 `tsconfig.json` 做路径映射等转换。类型擦除保留源码位置，通常不需要额外源码映射。[Node.js 24.12.0：TypeScript](https://nodejs.org/download/release/v24.12.0/docs/api/typescript.html)

建议起步约束为：ESM、普通 `async/await`、函数/对象和类型标注；使用 `.mts`，或者显式设置模块类型的 `.ts`。首版不支持依赖语法转换的 enum、参数属性、装饰器或 JSX，不支持任意 npm 依赖安装。对编写应对宏而言，这些限制通常不妨碍顺序流程和条件分支，但应通过真实宏确认。

SDK 若按包分发，应提供 JS 实现和 `.d.ts`，用户宏保留 TS；SDK 的发布构建由 Nyra 开发流程承担。Node 原生不会替用户处理 `node_modules` 中的 TS 源文件，因此不能直接将全部包源码交给运行时。[Node.js：依赖中的类型擦除限制](https://nodejs.org/api/typescript.html#type-stripping-in-dependencies)

编辑器类型检查与运行是两条路径。可以在启动宏前调用固定版本的 TypeScript 做 `noEmit` 校验，失败则不开始输入；它检查类型但不生成 JS 文件，不要求用户手工编译。[TypeScript：noEmit](https://www.typescriptlang.org/tsconfig/noEmit.html)

若真实宏需要更完整的 TS 转换，再考虑 Node + `tsx`；官方将其列为支持 TS 的可选工具。代价是增加依赖分发和源码映射验证，而不是自行实现语言。[Node.js：完整 TypeScript 支持](https://nodejs.org/api/typescript.html#full-typescript-support)

### 11.5 调试环境：基本断点便宜，执行状态同步更贵

基本方案是 Nyra 在开发模式启动带 Inspector 的 Node，输出本机调试地址，IDE 手动 attach。`--inspect-brk` 可让脚本在用户代码开始前暂停，避免还未连接调试器就发送输入。[Node.js：调试](https://nodejs.org/learn/getting-started/debugging)

VS Code 已提供 Node/TypeScript 调试和 attach 能力，可作为首个验收环境。这里估算的是配置和集成现有调试器；不是开发调试协议、IDE 插件或在 Nyra 浮窗里嵌入代码编辑器。用户当前使用的 RustRover 是否具备所需 JS 调试支持尚未验证，不将其计作现成可用能力。[VS Code：Node.js debugging](https://code.visualstudio.com/docs/nodejs/nodejs-debugging)

1～2 人日的基础调试范围不包含“任意断点都同步暂停宿主动作”。首版允许在脚本调用之间观察变量；若宿主已有按键或等待动作在执行，脚本断点不会自动冻结它，仍按原有截止时间与取消规则处理。开发模式可延长脚本启动/连接等待，但不能延长旧画面有效期或无限保持按键。

重新运行应显式停止上一轮，由宿主取消并完成清理，再新建运行。避免文件保存自动触发整段宏重跑，也不使用调试器的“重新执行当前函数”重放有副作用的步骤。跨 Rust/TS 的统一单步、协调暂停和 IDE 一键管理可后续设计，不能隐藏在“支持断点”这一小项里。

### 11.6 分发、维护与尚未计入的扩展

可分发首版在自用版之上增加 3～5 人日，范围为：固定 Node 和 SDK/类型检查工具版本、随包放置所需文件、模块路径与带空格/中文路径处理、附带必要许可证文件，以及在未安装开发工具的 Windows 环境检查正常运行。普通用户运行宏不需要安装 IDE；编辑和调试仍使用外部编辑器。这里只是便携目录分发，不含签名安装器、自动更新或跨平台支持。

运行时体积、Node 进程内存和启动耗时尚未测量，当前不承诺“轻量”或给出伪精确数值。引入 TS 会增加一个运行进程及工具链；是否接受该成本，应在 2～4 人日原型中实际测量。

后续维护主要来自协议/SDK同步、运行时更新和编辑器兼容。一次无破坏性版本升级，可暂按 0.5～2 人日检查预算考虑；这是条件性估算，不是固定每月成本，破坏性变更另计。固定受支持版本可缩小验证范围，不能永久替代更新维护。

不在 10～18 人日基线内的内容包括：自定义 IDE 扩展、内置编辑器、通用视觉定位工具、完整轨迹查看/离线回放、任意 npm 包管理、不可信脚本沙箱、多进程并发宏和 JS 引擎嵌入。这些都不是当前调用 SDK、直接运行 TS 和基本断点调试的必要前提。

### 11.7 建议的决策门槛

值得先投入一个有界原型，验证以下事实，而不是立刻批准整个 TS 集成工程：本地宏修改后可直接运行；跨进程等待能完成；源码断点/错误位置正确；取消或子进程退出能令宿主终止当前运行；启动延迟和附加资源占用可以接受。若尚无真实宿主能力，最后两项必须注明验证到哪一层。

原型通过后，再以 10～18 人日的自用版本范围推进。若仅有少量固定按键，TS 接入成本可能不值得；若应对方案会持续增加等待、分支和调试需求，成熟语言工具的收益更可能覆盖这笔投入。当前需求倾向后者，但技术选型仍应由真实流程和原型结果确认。

本节仅补充估算和范围定义，未安装运行时、生成 SDK/调试配置、修改业务代码或执行原型。官方资料用于核对工具能力，所有人日数字均为本次工程估算。
