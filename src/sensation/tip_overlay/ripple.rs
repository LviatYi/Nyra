...
use crate::settings_default_values::{
    TIP_OVERLAY_RIPPLE_EXPAND_DURATION, TIP_OVERLAY_RIPPLE_FADE_DURATION,
    TIP_OVERLAY_RIPPLE_SEGMENTS, WINDOW_HEIGHT, WINDOW_WIDTH,
};
...
impl Ripple {
    ...
    pub(super) fn begin(
        ...
        .into_iter()
        .map(|corner| self.origin.distance(corner))
        .fold(0.0, f32::max);
...
fn update_ripple_mesh(mesh: &mut Mesh, ripple: &Ripple, radius: f32) {
    ...
    for (index, direction) in ripple.directions.iter().enumerate() {
        let point = ripple.origin + *direction * radius;
